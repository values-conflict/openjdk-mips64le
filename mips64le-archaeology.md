...
## Step 2 — mips64le Port in jdk17u

The mips64le backend is **fully present and actively maintained in all three repos**. It was never removed.
...
Both big-endian `mips64` and little-endian `mips64el` share the same `cpu/mips/` backend. Loongson's machines historically ran `mips64el`.
...
`os_linux_mips.cpp` (31 KB) handles signal stack setup, thread context, and CPU feature detection. Note `safefetch_linux_mips64.S` — filename carries explicit `mips64` confirming 64-bit target.

### `src/jdk.hotspot.agent/` — 15 Java files (SA agent)

Full Serviceability Agent support: `MachineDescriptionMIPS64.java`, `LinuxMIPS64CFrame.java`, `MIPS64Frame.java`, `MIPS64RegisterMap.java`, `LinuxMIPS64JavaThreadPDAccess.java`, etc.
...
Both architectures are always updated in the same commits. The most recent commit (2026-05-07) touched mips and loongarch together.
...
"Initial Linux/LoongArch64 Port — A complete (i.e., template interpreter,
...
The loongarch port is substantially larger due to C1, JVMCI, ZGC, Panama FFI, Loom, and richer crypto intrinsics. The `.ad` file is 57% larger from additional C2 intrinsics and vector patterns.
...
Note: the SA (Serviceability Agent / `jdk.hotspot.agent`) was removed in JDK 21+. jdk25u has no SA for any architecture.

### New files jdk25u loongarch has that mips64 (jdk17u) lacks — 45 files

| Category | Files | Notes for porting |
| --- | --- | --- |
| C1 JIT | 13 | Entirely new; adapt from loongarch C1 to MIPS64 ISA |
...
**jdk17u is the more complete reference for forward-porting.** jdk11u is useful as a cross-reference for understanding what was truly JDK-11-era vs JDK-17-era additions to the mips port.
...
- **Identical file layout.** Every mips file has a direct loongarch counterpart: `mips_64.ad` ↔ `loongarch_64.ad`, `interp_masm_mips_64.cpp` ↔ `interp_masm_loongarch_64.cpp`, `sharedRuntime_mips_64.cpp` ↔ `sharedRuntime_loongarch_64.cpp`, etc.
...
A **hybrid forward-port** in two phases:
...
2. Use the jdk17u→jdk25u loongarch delta as the exact API-change guide. Every upstream JVM API change reflected in jdk25u's loongarch port (vs jdk17u's loongarch port) needs to be applied to the mips port with MIPS64 ISA instructions.
...
5. Add stack-chunk inline headers: `smallRegisterMap_mips.inline.hpp`, `stackChunkFrameStream_mips.inline.hpp`, `stackChunkOop_mips.inline.hpp` — translate from loongarch equivalents.
6. Build with `--with-jvm-features=-c1,-zgc,-shenandoahgc` to defer unsupported features.

**Phase 2 — C1 JIT (4–8 weeks, optional):**

7. Adapt the 13 jdk17u loongarch `c1_*.cpp/.hpp` files to MIPS64 ISA. The C1 LIR assembler (`c1_LIRAssembler_loongarch_64.cpp`, 112 KB) needs instruction-level translation. Feasible because LoongArch and MIPS64 are both clean RISC load-store ISAs with 32 registers and similar calling conventions.

**Phase 3 — ZGC, Shenandoah, Panama (deferred, can be added incrementally).**

The jdk25u loongarch implementations serve as templates throughout. This is not a blind copy-replace — MIPS64 assembly must be rewritten — but the structure and algorithm are correct starting points.

### Q4: Was jdk25u's LoongArch port derived from the mips work?

**Strong circumstantial evidence, effectively conclusive:**

1. **Same author.** aoqi (`aoqi@loongson.cn`) authored the jdk25u initial LoongArch commit (2022-08-11) and had been fixing MIPS-specific OpenJDK bugs since at least 2018 (`8200213: Configure broken on MIPS`, `8204091: Configure broken on MIPS when uname returns mipsel or mips64el`).
2. **Same copyright year.** Both backends carry `Copyright (c) 2015, Loongson Technology` — the year Loongson started their MIPS port.
3. **Identical structural skeleton.** The file layout, function names, and architectural patterns are too consistent to be coincidental independent implementations. Particularly `beq_far`/`bne_far`/`b_far` — a Loongson-internal abstraction for limited branch offsets, not an upstream OpenJDK pattern — appears identically named in both.
