...
bool NativeInstruction::is_float_branch() {
  if (!is_op(Assembler::cop1_op) ||
      !is_rs(as_Register(Assembler::bc1f_op))) return false;
...
address NativeCall::destination() const {
  ...
  if (destination > addr) {
    ...
    CodeBlob* cb = CodeCache::find_blob(addr);   // Else we get assertion if nmethod is zombie.
...
address NativeJump::jump_destination() {
  ...
  // patchable_set48(dst, simm16) + jr(dst) + nop (compact form)
  // daddiu dst, R0, imm16
  // jr dst
  // nop
  if (is_op(Assembler::daddiu_op) &&
      is_special_op(int_at(4), Assembler::jr_op) &&
      nativeInstruction_at(addr_at(8))->is_nop()) {
    int sign = int_at(0) & 0x8000;
    if (sign == 0) {
      return (address)(intptr_t)(int16_t)(int_at(0) & 0xffff);
    } else {
      return (address)(intptr_t)(int16_t)(int_at(0) & 0xffff);
    }
  }
...
bool NativeInstruction::is_jump()
{
  ...
  // patchable_set48 for simm16 values: daddiu + jr + nop (compact form)
  if (is_op(Assembler::daddiu_op) &&
      is_special_op(int_at(4), Assembler::jr_op) &&
      nativeInstruction_at(addr_at(8))->is_nop()) {
    return true;
