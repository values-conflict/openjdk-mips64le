...
void SharedRuntime::generate_i2c2i_adapters(MacroAssembler *masm,
  ...
  // In jdk25u, max_arg replaces comp_args_on_stack
  int comp_args_on_stack = max_arg;
  ...
  Label skip_fixup;
  {
    ...
    // jdk25u: use CompiledICData instead of CompiledICHolder
    __ ic_check(1 /* end_alignment */);
    __ ld_ptr(Rmethod, holder, in_bytes(CompiledICData::speculated_method_offset()));
...
nmethod *SharedRuntime::generate_native_wrapper(MacroAssembler* masm,
  ...
  bool is_critical_native = false;  // critical_entry removed in jdk25u
  ...
  if (!is_critical_native) {
    ...
    __ sw(R0, AT, in_bytes(JNIHandleBlock::top_offset()));
  }
...
void SharedRuntime::generate_deopt_blob() {
  ...
  __ lw(T8, unroll, in_bytes(Deoptimization::UnrollBlock::size_of_deoptimized_frame_offset()));
  __ addu(SP, SP, T8);
  // sp should be pointing at the return address to the caller (3)

  // Load array of frame pcs into pcs
  __ ld_ptr(pcs, unroll, in_bytes(Deoptimization::UnrollBlock::frame_pcs_offset()));
  __ addiu(SP, SP, wordSize);  // trash the old pc
  // Load array of frame sizes into T6
  __ ld_ptr(sizes, unroll, in_bytes(Deoptimization::UnrollBlock::frame_sizes_offset()));
  ...
  __ lw(TSR, unroll, in_bytes(Deoptimization::UnrollBlock::total_frame_sizes_offset()));
  __ bang_stack_size(TSR, T8);
#endif

  // Load count of frams into T3
  __ lw(count, unroll, in_bytes(Deoptimization::UnrollBlock::number_of_frames_offset()));
  ...
  __ lw(AT, unroll, in_bytes(Deoptimization::UnrollBlock::caller_adjustment_offset()));
  __ subu(SP, SP, AT);
...
void SharedRuntime::generate_uncommon_trap_blob() {
...
#ifdef ASSERT
  { Label L;
    __ ld_ptr(AT, unroll, in_bytes(Deoptimization::UnrollBlock::unpack_kind_offset()));
  ...
  __ ld(pcs, unroll, in_bytes(Deoptimization::UnrollBlock::frame_pcs_offset()));
...
SafepointBlob* SharedRuntime::generate_handler_blob(SharedStubId id, address call_ptr) {
  int pool_type = (int)id; // compatibility
...
RuntimeStub* SharedRuntime::generate_resolve_blob(SharedStubId id, address destination) {
  ...
  __ get_thread(thread);
#endif
  __ st_ptr(R0, thread, in_bytes(JavaThread::vm_result_oop_offset()));
...
RuntimeStub* SharedRuntime::generate_throw_exception(SharedStubId id, address runtime_entry) {
  ...
  // Build a minimal stub: save minimal state, call runtime, restore
  enum layout {
    thread_off,
    FP_off,
    ret_address,
  ...
  const int insts_size = 2048;
  const int locs_size  = 32;
  ...
  __ addiu(SP, SP, (-1) * (framesize-2) * wordSize);

  int frame_complete = __ offset();

  __ sd(java_thread, SP, thread_off * wordSize);
  if (java_thread != RA0) __ move(RA0, java_thread);

  __ set_last_Java_frame(java_thread, SP, FP, nullptr);
