...
void InterpreterMacroAssembler::get_cache_index_at_bcp(Register index,
  ...
  if (index_size == sizeof(u2)) {
    ...
    // JDK 25: invokedynamic index stored directly as a u4 (not as ~index).
    // Just read the raw value; no ~-decode needed.
...
void InterpreterMacroAssembler::load_resolved_indy_entry(Register cache, Register index) {
  ...
  ld(cache, Address(cache, in_bytes(ConstantPoolCache::invokedynamic_entries_offset())));
  // Scale the index to be the entry index * sizeof(ResolvedIndyEntry)
  dsll(index, index, log2i_exact(sizeof(ResolvedIndyEntry)));
  daddiu(cache, cache, Array<ResolvedIndyEntry>::base_offset_in_bytes());
  daddu(cache, cache, index);
}

void InterpreterMacroAssembler::load_field_entry(Register cache, Register index, int bcp_offset) {
  // Get index out of bytecode pointer
  get_cache_index_at_bcp(index, bcp_offset, sizeof(u2));
  // Scale the index by sizeof(ResolvedFieldEntry)
  if (is_power_of_2(sizeof(ResolvedFieldEntry))) {
    dsll(index, index, log2i_exact(sizeof(ResolvedFieldEntry)));
  } else {
    li64(cache, sizeof(ResolvedFieldEntry));
    dmultu(index, cache);
    mflo(index);
  }
  ld(cache, FP, frame::interpreter_frame_cache_offset * wordSize);
  // Get address of field entries array
  ld(cache, Address(cache, in_bytes(ConstantPoolCache::field_entries_offset())));
  daddiu(cache, cache, Array<ResolvedFieldEntry>::base_offset_in_bytes());
  daddu(cache, cache, index);
  if (os::is_MP()) sync(); // load acquire
}

void InterpreterMacroAssembler::load_method_entry(Register cache, Register index, int bcp_offset) {
  // Get index out of bytecode pointer
  get_cache_index_at_bcp(index, bcp_offset, sizeof(u2));
  li64(cache, sizeof(ResolvedMethodEntry));
  dmultu(index, cache);
  mflo(index);

  ld(cache, FP, frame::interpreter_frame_cache_offset * wordSize);
  // Get address of method entries array
  ld(cache, Address(cache, in_bytes(ConstantPoolCache::method_entries_offset())));
  daddiu(cache, cache, Array<ResolvedMethodEntry>::base_offset_in_bytes());
  daddu(cache, cache, index);
}
...
void InterpreterMacroAssembler::load_resolved_reference_at_index(
  ...
  ld(result, result, in_bytes(ConstantPool::cache_offset()));
  ld(result, result, in_bytes(ConstantPoolCache::resolved_references_offset()));
...
void InterpreterMacroAssembler::load_resolved_klass_at_index(Register cpool,
  ...
  ld_ptr(resolved_klasses, Address(cpool, in_bytes(ConstantPool::resolved_klasses_offset())));
  dsll(AT, index, Address::times_ptr);
...
void InterpreterMacroAssembler::load_resolved_method_at_index(int byte_no,
                                                              ...
                                                              Register cache,
                                                              Register index) {
  // cache must point to a ResolvedMethodEntry (set by load_method_entry)
  ld(method, Address(cache, in_bytes(ResolvedMethodEntry::method_offset())));
}
...
void InterpreterMacroAssembler::remove_activation(
  ...
  ld(T1, c_rarg0, in_bytes(BasicObjectLock::obj_offset()));
  bne(T1, R0, unlock);
  delayed()->nop();
  pop(state);
  if (throw_monitor_exception) {
  ...
  // Load return address into RA before restoring FP/SP
  ld(RA, FP, frame::interpreter_frame_return_addr_offset * wordSize);
...
void InterpreterMacroAssembler::lock_object(Register lock_reg) {
  ...
  // Always use the runtime call — the LM_LIGHTWEIGHT inline fast path here was JDK 17
  // stack-locking code that doesn't work with JDK 25's lightweight-locking protocol.
...
void InterpreterMacroAssembler::profile_return_type(Register mdp, Register ret, Register tmp) {
  ...
  if (ProfileInterpreter && MethodData::profile_return()) {
    ...
    if (MethodData::profile_return_jsr292_only()) {
      ...
      get_method(tmp);
      lhu(tmp, tmp, in_bytes(Method::intrinsic_id_offset()));
