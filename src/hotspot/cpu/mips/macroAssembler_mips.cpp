...
int MacroAssembler::ic_check(int end_alignment) {
  ...
  if (UseCompressedClassPointers) {
    ...
    lwu(tmp2, Address(data, in_bytes(CompiledICData::speculated_klass_offset())));
  } else {
    ld(tmp1, Address(receiver, oopDesc::klass_offset_in_bytes()));
    ld(tmp2, Address(data, in_bytes(CompiledICData::speculated_klass_offset())));
  }
  ...
  patchable_jump(SharedRuntime::get_ic_miss_stub());
  bind(ic_hit);
...
// biased_locking_enter/exit removed (BiasedLocking removed in JDK 18)
...
void MacroAssembler::debug(char* msg/*, RegistersForDebugging* regs*/) {
  if ( ShowMessageBoxOnError ) {
    ...
    // ThreadStateTransition::transition removed in jdk25u; use from_vm instead
    if (saved_state == _thread_in_Java) {
      ThreadStateTransition::transition_from_vm(JavaThread::current(), _thread_in_Java);
    }
  }
  else
...
void MacroAssembler::call_VM_base(Register oop_result,
  ...
  if (oop_result->is_valid()) {
    ld(oop_result, java_thread, in_bytes(JavaThread::vm_result_oop_offset()));
    sd(R0, java_thread, in_bytes(JavaThread::vm_result_oop_offset()));
...
void MacroAssembler::incr_allocated_bytes(Register thread,
  ...
  // Note: _allocated_bytes is private; use in_bytes() via public offset if available,
  // otherwise use a runtime call to incr_allocated_bytes.
  // For now, stub this out since tlab_allocate in jdk25u doesn't manually track it.
  (void)var_size_in_bytes; (void)con_size_in_bytes;
  Unimplemented();
}
...
void MacroAssembler::load_mirror(Register mirror, Register method, Register tmp) {
  ...
  ld_ptr(mirror, mirror, in_bytes(ConstantPool::pool_holder_offset()));
...
void MacroAssembler::load_method_holder(Register holder, Register method) {
  ...
  ld(holder, Address(holder, in_bytes(ConstantPool::pool_holder_offset()))); // InstanceKlass*
}
...
void MacroAssembler::access_store_at(BasicType type, DecoratorSet decorators, Address dst, Register src,
  ...
  bool as_raw = (decorators & AS_RAW) != 0;
  if (as_raw) {
    bs->BarrierSetAssembler::store_at(this, decorators, type, dst, src, tmp1, tmp2, noreg);
  } else {
    bs->store_at(this, decorators, type, dst, src, tmp1, tmp2, noreg);
  }
}
...
void MacroAssembler::encode_klass_not_null(Register r) {
  ...
  if (CompressedKlassPointers::shift() != 0) {
    assert (CompressedKlassPointers::shift() == CompressedKlassPointers::shift(), "decode alg wrong");
    shr(r, CompressedKlassPointers::shift());
  }
}
...
void MacroAssembler::encode_klass_not_null(Register dst, Register src) {
  if (dst == src) {
    ...
    if (CompressedKlassPointers::base() != NULL) {
      ...
      if (CompressedKlassPointers::shift() != 0) {
        assert (CompressedKlassPointers::shift() == CompressedKlassPointers::shift(), "decode alg wrong");
        shr(dst, CompressedKlassPointers::shift());
      }
    } else {
      if (CompressedKlassPointers::shift() != 0) {
        assert (CompressedKlassPointers::shift() == CompressedKlassPointers::shift(), "decode alg wrong");
        dsrl(dst, src, CompressedKlassPointers::shift());
      } else {
        move(dst, src);
      }
    }
...
void  MacroAssembler::decode_klass_not_null(Register r) {
  ...
  if (CompressedKlassPointers::shift() != 0) {
    assert(CompressedKlassPointers::shift() == CompressedKlassPointers::shift(), "decode alg wrong");
    shl(r, CompressedKlassPointers::shift());
  }
  if (CompressedKlassPointers::base() != NULL) {
    set64(AT, (int64_t)CompressedKlassPointers::base());
    daddu(r, r, AT);
  ...
  if (UseCompressedOops || UseCompressedClassPointers) {
    if (Universe::heap() != NULL) {
      if (CompressedOops::base() == NULL) {
        ...
        set64(S5_heapbase, (int64_t)(address)CompressedOops::base());
      }
    } else {
      set64(S5_heapbase, (intptr_t)CompressedOops::base_addr());
...
void MacroAssembler::get_vm_result(Register oop_result, Register java_thread) {
  ld(oop_result, Address(java_thread, in_bytes(JavaThread::vm_result_oop_offset())));
  sd(R0, Address(java_thread, in_bytes(JavaThread::vm_result_oop_offset())));
  verify_oop(oop_result, "broken oop in call_VM_base");
}

void MacroAssembler::get_vm_result_2(Register metadata_result, Register java_thread) {
  ld(metadata_result, Address(java_thread, in_bytes(JavaThread::vm_result_metadata_offset())));
  sd(R0, Address(java_thread, in_bytes(JavaThread::vm_result_metadata_offset())));
}
...
void MacroAssembler::lookup_interface_method(Register recv_klass,
  ...
  for (int peel = 1; peel >= 0; peel--) {
    ld(method_result, Address(scan_temp, in_bytes(itableOffsetEntry::interface_offset())));
  ...
  if (return_method) {
    // Got a hit.
    lw(scan_temp, Address(scan_temp, in_bytes(itableOffsetEntry::offset_offset())));
    if (UseLEXT1) {
...
void MacroAssembler::lookup_virtual_method(Register recv_klass,
  ...
  set64(tmp, base + in_bytes(vtableEntry::method_offset()));
  daddu(tmp, tmp, AT);
...
void MacroAssembler::clear_jweak_tag(Register possibly_jweak) {
  const uintptr_t jweak_tag_mask = JNIHandles::TypeTag::weak_global;
  // Clear the weak tag bit
  move(AT, ~(int64_t)jweak_tag_mask);
...
void MacroAssembler::resolve_jobject(Register value,
  ...
  delayed()->nop();
  move(AT, (int64_t)JNIHandles::TypeTag::weak_global); // Test for jweak tag.
  andr(AT, value, AT);
  beq(AT, R0, not_weak);
  delayed()->nop();
  // Resolve jweak.
  access_load_at(T_OBJECT, IN_NATIVE | ON_PHANTOM_OOP_REF,
                 value, Address(value, -(intptr_t)JNIHandles::TypeTag::weak_global), tmp, thread);
  verify_oop(value);
  b(done);
  delayed()->nop();
  bind(not_weak);
