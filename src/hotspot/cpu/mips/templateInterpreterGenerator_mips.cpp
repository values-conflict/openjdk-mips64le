...
address TemplateInterpreterGenerator::generate_math_entry(AbstractInterpreter::MethodKind kind) {
  ...
  case Interpreter::java_lang_math_exp :
    ...
    {
      // transcendental entry removed in jdk25u; call stub directly
      address fn = nullptr;
      if (kind == Interpreter::java_lang_math_sin) fn = StubRoutines::dsin() ? StubRoutines::dsin() : CAST_FROM_FN_PTR(address, SharedRuntime::dsin);
      else if (kind == Interpreter::java_lang_math_cos) fn = StubRoutines::dcos() ? StubRoutines::dcos() : CAST_FROM_FN_PTR(address, SharedRuntime::dcos);
      else if (kind == Interpreter::java_lang_math_tan) fn = StubRoutines::dtan() ? StubRoutines::dtan() : CAST_FROM_FN_PTR(address, SharedRuntime::dtan);
      else if (kind == Interpreter::java_lang_math_log) fn = StubRoutines::dlog() ? StubRoutines::dlog() : CAST_FROM_FN_PTR(address, SharedRuntime::dlog);
      else if (kind == Interpreter::java_lang_math_log10) fn = StubRoutines::dlog10() ? StubRoutines::dlog10() : CAST_FROM_FN_PTR(address, SharedRuntime::dlog10);
      else if (kind == Interpreter::java_lang_math_exp) fn = StubRoutines::dexp() ? StubRoutines::dexp() : CAST_FROM_FN_PTR(address, SharedRuntime::dexp);
      if (fn) __ call_VM_leaf(fn, 1);
    }
    __ dmfc1(SP, F25);
    __ dmfc1(RA, F24);
    break;
  case Interpreter::java_lang_math_pow :
    ...
    __ dins(SP, R0, 0, exact_log2(StackAlignmentInBytes));
    {
      address fn = StubRoutines::dpow() ? StubRoutines::dpow() : CAST_FROM_FN_PTR(address, SharedRuntime::dpow);
      __ call_VM_leaf(fn, 2);
    }
    __ dmfc1(SP, F25);
    __ dmfc1(RA, F24);
    break;
...
address TemplateInterpreterGenerator::generate_return_entry_for(TosState state, int step, size_t index_size) {
  ...
  // Load number of parameters from the CP cache entry to advance SP past the arguments.
  // invokedynamic uses a 4-byte bytecode operand and ResolvedIndyEntry; others use 2-byte + ResolvedMethodEntry.
  const Register cache = T9;
  const Register index = T3;
  const Register param_size = cache;
  if (index_size == sizeof(u4)) {
    // invokedynamic return entry: read from ResolvedIndyEntry
    __ load_resolved_indy_entry(cache, index);
    __ lhu(param_size, Address(cache, in_bytes(ResolvedIndyEntry::num_parameters_offset())));
  } else {
    __ load_method_entry(cache, index);
    __ lhu(param_size, Address(cache, in_bytes(ResolvedMethodEntry::num_parameters_offset())));
  }
  __ dsll(AT, param_size, Interpreter::logStackElementSize);
  __ daddu(SP, SP, AT);
...
void TemplateInterpreterGenerator::generate_stack_overflow_check(void) {
  ...
  __ move(SP, Rsender);
  assert(SharedRuntime::throw_StackOverflowError_entry() != NULL, "stub not yet generated");
...
void TemplateInterpreterGenerator::lock_method(void) {
  ...
  __ sd(T0, SP, in_bytes(BasicObjectLock::obj_offset()));   // store object
...
void TemplateInterpreterGenerator::generate_fixed_frame(bool native_call) {
  ...
  // Store relativized LVP: (LVP - FP) / wordSize, see frame::interpreter_frame_locals()
  __ dsubu(AT, LVP, FP);
  __ dsrl(AT, AT, Interpreter::logStackElementSize);
  __ sd(AT, FP, (-++i) * wordSize);  // save locals offset (relativized)
  ...
  __ ld(T2, T2, in_bytes(ConstantPool::cache_offset()));
...
address TemplateInterpreterGenerator::generate_native_entry(bool synchronized) {
  ...
  bool inc_counter  = UseCompiler || CountCompiledCalls || false; // LogTouchedMethods removed
  // Rsender: sender's sp
  // Rmethod: Method*
  address entry_point = __ pc();
...
#define PARA_MASK  0xf
  ...
  __ sw(R0, t, in_bytes(JNIHandleBlock::top_offset()));
  ...
  // do unlocking if necessary
  {
    ...
    // the code below should be shared with interpreter macro assembler implementation
    {
      ...
      __ ld(t, c_rarg0, in_bytes(BasicObjectLock::obj_offset()));
      __ bne(t, R0, unlock);
      __ delayed()->nop();
...
void TemplateInterpreterGenerator::generate_throw_exception() {
...
#ifndef CORE
  {
    ...
    __ remove_activation(vtos, false, false, false);
  ...
  __ get_thread(thread);
#endif
  __ sd(T0, thread, in_bytes(JavaThread::vm_result_oop_offset()));
  ...
  // FP: fp of caller
  // After remove_activation, RA holds the frame's return address (jdk25u; jdk17u used ret_addr arg).
