...
// Stack argument bias: no shadow space on MIPS64 n64 ABI.
// Stack inputs to pd_add_offset_to_oop are at FP + offset (no bias).
static constexpr int FP_BIAS = 0;
...
void DowncallLinker::StubGenerator::pd_add_offset_to_oop(VMStorage reg_oop, VMStorage reg_offset,
  ...
  if (reg_oop.is_reg()) {
    ...
    if (reg_offset.is_reg()) {
      ...
      __ daddu(reg_oop_reg, reg_oop_reg, as_Register(reg_offset));
    } else {
      ...
      __ daddu(reg_oop_reg, reg_oop_reg, r_tmp1);
    }
  } else {
...
void DowncallLinker::StubGenerator::generate() {
  ...
  // tmp1 and tmp2 are caller-save scratch registers.
  Register tmp1 = T8;  // r24 -- never an argument or return register
  Register tmp2 = V1;  // r3  -- second integer return (safe after call)
  ...
  assert(_abi._shadow_space_bytes == 0, "not expecting shadow space on MIPS64");
  ...
  // RA and FP are already in place; allocate remaining frame slots.
  int extra_bytes = ((int)_frame_size_slots - 4) << LogBytesPerInt;
  assert(is_simm16(-extra_bytes), "frame too large for daddiu");
  __ daddiu(SP, SP, -extra_bytes);

  _frame_complete = __ pc() - start;

  if (_needs_transition) {
    address the_pc = __ pc();
    __ set_last_Java_frame(SP, FP, the_pc);
    OopMap* map = new OopMap(_frame_size_slots, 0);
    _oop_maps->add_gc_map(the_pc - start, map);

    // State transition: _thread_in_Java → _thread_in_native
    __ li(tmp1, _thread_in_native);
    if (os::is_MP()) {
      __ sync(); // release
    }
    __ sw(tmp1, TREG, in_bytes(JavaThread::thread_state_offset()));
  }
  ...
  __ delayed()->nop();
  // this call is assumed not to have killed TREG

  if (_needs_return_buffer) {
    __ ld(tmp1, SP, locs.data_offset(StubLocations::RETURN_BUFFER));
    int offset = 0;
    for (int i = 0; i < _output_registers.length(); i++) {
      VMStorage reg = _output_registers.at(i);
      if (reg.type() == StorageType::INTEGER) {
        __ sd(as_Register(reg), tmp1, offset);
        offset += 8;
      } else if (reg.type() == StorageType::FLOAT) {
        __ sdc1(as_FloatRegister(reg), tmp1, offset);
        offset += 8;
      } else {
        ShouldNotReachHere();
      }
  ...
  if (_captured_state_mask != 0) {
    ...
    __ ld(RA0, Address(SP, locs.data_offset(StubLocations::CAPTURED_STATE_BUFFER)));
    __ li(RA1, _captured_state_mask);
  ...
  if (_needs_transition) {
    // Transition: _thread_in_native → _thread_in_native_trans
    __ li(tmp1, _thread_in_native_trans);
    if (os::is_MP()) {
      __ sync(); // release + acquire
    }
    __ sw(tmp1, TREG, in_bytes(JavaThread::thread_state_offset()));

    if (os::is_MP()) __ sync(); // force write before safepoint read

    __ safepoint_poll_acquire(L_safepoint_poll_slow_path, TREG);
    __ lw(tmp1, TREG, in_bytes(JavaThread::suspend_flags_offset()));
    __ bne(tmp1, R0, L_safepoint_poll_slow_path);
    __ delayed()->nop();

    __ bind(L_after_safepoint_poll);

    // Transition: _thread_in_native_trans → _thread_in_Java
    ...
    __ lw(tmp1, TREG, in_bytes(JavaThread::stack_guard_state_offset()));
    __ addiu(tmp1, tmp1, -(int)StackOverflow::stack_guard_yellow_reserved_disabled);
    __ beq(tmp1, R0, L_reguard);
    __ delayed()->nop();
