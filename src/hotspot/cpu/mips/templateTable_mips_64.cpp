  ...
  case Bytecodes::_fast_sputfield:
    {
      ...
      if (byte_no == f1_byte) {
        __ daddiu(tmp_reg, tmp_reg, in_bytes(ResolvedFieldEntry::get_code_offset()));
      } else {
        __ daddiu(tmp_reg, tmp_reg, in_bytes(ResolvedFieldEntry::put_code_offset()));
      }
      // Load-acquire the bytecode to match store-release in ResolvedFieldEntry::fill_in()
      __ sync();
      __ lbu(tmp_reg, tmp_reg, 0);
      __ sync();
...
void TemplateTable::ldc(LdcType type) {
  ...
  __ bind(call_ldc);
  __ move(A1, is_ldc_wide(type) ? 1 : 0);
...
void TemplateTable::condy_helper(Label& Done) {
  ...
  __ dsrl(flags, flags, ConstantPoolCache::tos_state_shift);
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
  ...
  if (UseLoopCounter && UseOnStackReplacement) {
    ...
    __ lb(T3, V0, in_bytes(nmethod::state_offset()));
    ...
    __ ld(AT, Rnext, in_bytes(nmethod::osr_entry_point_offset()));
    __ jr(AT);
    __ delayed()->nop();
  }
}
...
void TemplateTable::_return(TosState state) {
  ...
  if (_desc->bytecode() == Bytecodes::_return_register_finalizer) {
    ...
    // Check has_finalizer flag via KlassFlags
    __ lw(LVP, LVP, in_bytes(Klass::misc_flags_offset()));
    __ move(AT, KlassFlags::_misc_has_finalizer);
    __ andr(AT, AT, LVP);
  ...
  __ remove_activation(state);  // loads return addr into RA
  __ sync();
...
void TemplateTable::resolve_cache_and_index_for_method(int byte_no,
  ...
  switch (byte_no) {
    case f1_byte:
      __ daddiu(temp, Rcache, in_bytes(ResolvedMethodEntry::bytecode1_offset()));
      break;
    case f2_byte:
      __ daddiu(temp, Rcache, in_bytes(ResolvedMethodEntry::bytecode2_offset()));
      break;
  }
  // Load-acquire the bytecode
  __ sync();
  __ lbu(temp, temp, 0);
  __ sync();
  // is resolved?
  int i = (int)code;
  __ daddiu(temp, temp, -i);
...
void TemplateTable::resolve_cache_and_index_for_field(int byte_no,
  ...
  if (byte_no == f1_byte) {
    __ daddiu(temp, Rcache, in_bytes(ResolvedFieldEntry::get_code_offset()));
  } else {
    __ daddiu(temp, Rcache, in_bytes(ResolvedFieldEntry::put_code_offset()));
  }
  // Load-acquire the bytecode
  __ sync();
  __ lbu(temp, temp, 0);
...
// New API: cache must already point to a ResolvedFieldEntry
void TemplateTable::load_resolved_field_entry(Register obj,
  ...
  assert_different_registers(cache, tos_state, flags, off);

  // Field offset
  __ lw(off, Address(cache, in_bytes(ResolvedFieldEntry::field_offset_offset())));

  // Flags
  __ lbu(flags, Address(cache, in_bytes(ResolvedFieldEntry::flags_offset())));

  // TOS state
  if (tos_state != noreg) {
    __ lbu(tos_state, Address(cache, in_bytes(ResolvedFieldEntry::type_offset())));
  }

  // klass overwrite register
  if (is_static) {
    __ ld(obj, Address(cache, in_bytes(ResolvedFieldEntry::field_holder_offset())));
    const int mirror_offset = in_bytes(Klass::java_mirror_offset());
    __ ld(obj, Address(obj, mirror_offset));
    __ resolve_oop_handle(obj, T9, AT);
  }
}

// get the method, itable_index and flags of the current invoke
// Updated to use ResolvedMethodEntry (JDK-8302708)
void TemplateTable::load_invoke_cp_cache_entry(int byte_no,
  ...
  if (is_invokedynamic) {
    ...
    if (os::is_MP()) __ sync();  // load-acquire: ensure full entry visibility
    // If method is null the entry is not yet resolved; call resolve_from_cache.
    // (index/cache are scratch here; reload everything after the call.)
    {
      Label resolved;
      __ bne(method, R0, resolved);
      __ delayed()->nop();
      __ li(AT, (int)Bytecodes::_invokedynamic);
      __ call_VM(NOREG, CAST_FROM_FN_PTR(address, InterpreterRuntime::resolve_from_cache), AT);
      // Reload: call_VM clobbers T1-T9 (caller-save on MIPS)
      __ load_resolved_indy_entry(cache, index);
      __ ld(method, Address(cache, in_bytes(ResolvedIndyEntry::method_offset())));
      __ bind(resolved);
    }
    __ lbu(flags, Address(cache, in_bytes(ResolvedIndyEntry::flags_offset())));
    // Load resolved_references_index into itable_index for appendix lookup in prepare_invoke
    if (itable_index != NOREG) {
      __ lhu(itable_index, Address(cache, in_bytes(ResolvedIndyEntry::resolved_references_index_offset())));
    }
    // JDK-8302708: result_type is a separate byte; insert into bits [tos_state_shift+3..tos_state_shift]
    // so that prepare_invoke's dsrl(flags, tos_state_shift) extracts the correct TOS state.
    __ lbu(AT, Address(cache, in_bytes(ResolvedIndyEntry::result_type_offset())));
    __ dsll(AT, AT, ConstantPoolCache::tos_state_shift);
    __ orr(flags, flags, AT);
  } else {
    resolve_cache_and_index_for_method(byte_no, cache, index);
    __ lbu(flags, Address(cache, in_bytes(ResolvedMethodEntry::flags_offset())));
    // JDK-8302708: load method/klass/index into method and itable_index registers.
    // Each invoke type uses different fields:
    //   invokevirtual:   method = vtable_index (non-vfinal) or Method* (vfinal)
    //   invokeinterface: method = klass (non-forced-virtual) or unused (forced-virtual);
    //                    itable_index = Method* (non-forced-virtual) or vtable_index/Method* (forced-virtual)
    //   others:          method = Method*; itable_index unused (NOREG)
    if (is_invokevirtual) {
      Label is_vfinal_virt, done_virt;
      __ andi(AT, flags, (1 << ResolvedMethodEntry::is_vfinal_shift));
      __ bne(AT, R0, is_vfinal_virt);
      __ delayed()->nop();
      // non-vfinal: vtable index into method register
      __ lhu(method, Address(cache, in_bytes(ResolvedMethodEntry::table_index_offset())));
      __ b(done_virt);
      __ delayed()->nop();
      __ bind(is_vfinal_virt);
      // vfinal: Method* into method register
      __ ld(method, Address(cache, in_bytes(ResolvedMethodEntry::method_offset())));
      __ bind(done_virt);
    } else if (itable_index != NOREG && bytecode() == Bytecodes::_invokeinterface) {
      // invokeinterface: method(T2) = klass, itable_index(Rmethod) = Method*
      // For forced_virtual (Object methods via interface): itable_index = vtable_index or Method*
      Label is_forced_virt, done_intf;
      __ andi(AT, flags, (1 << ResolvedMethodEntry::is_forced_virtual_shift));
      __ bne(AT, R0, is_forced_virt);
      __ delayed()->nop();
      // non-forced-virtual: T2 = interface klass, Rmethod = Method*
      __ ld(method, Address(cache, in_bytes(ResolvedMethodEntry::klass_offset())));
      __ ld(itable_index, Address(cache, in_bytes(ResolvedMethodEntry::method_offset())));
      __ b(done_intf);
      __ delayed()->nop();
      __ bind(is_forced_virt);
      // forced_virtual: Rmethod = vtable index (non-vfinal) or Method* (vfinal)
      {
        Label is_vfinal_fv;
        __ andi(AT, flags, (1 << ResolvedMethodEntry::is_vfinal_shift));
        __ bne(AT, R0, is_vfinal_fv);
        __ delayed()->nop();
        __ lhu(itable_index, Address(cache, in_bytes(ResolvedMethodEntry::table_index_offset())));
        __ b(done_intf);
        __ delayed()->nop();
        __ bind(is_vfinal_fv);
        __ ld(itable_index, Address(cache, in_bytes(ResolvedMethodEntry::method_offset())));
      }
      __ bind(done_intf);
    } else {
      // invokespecial, invokestatic, invokehandle: load Method* into method register
      __ ld(method, Address(cache, in_bytes(ResolvedMethodEntry::method_offset())));
      // invokehandle also needs itable_index = resolved_references_index for appendix loading
      if (itable_index != NOREG) {
        __ lhu(itable_index, Address(cache, in_bytes(ResolvedMethodEntry::resolved_references_index_offset())));
      }
    }
    // JDK-8302708: _tos_state is a separate byte from _flags; insert into bits [tos_state_shift+3..tos_state_shift]
    // so that prepare_invoke's dsrl(flags, tos_state_shift) extracts the correct TOS state.
    __ lbu(AT, Address(cache, in_bytes(ResolvedMethodEntry::type_offset())));
    __ dsll(AT, AT, ConstantPoolCache::tos_state_shift);
    __ orr(flags, flags, AT);
  }
}
...
void TemplateTable::jvmti_post_field_access(Register cache, Register index,
  ...
  if (JvmtiExport::can_post_field_access()) {
    ...
    // tmp2: ResolvedFieldEntry pointer
    __ call_VM(NOREG, CAST_FROM_FN_PTR(address,
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static, RewriteControl rc) {
  ...
  const Register tos_state = index; // T0, reused after resolve
  ...
  {
    __ move(scratch, 1 << ResolvedFieldEntry::is_volatile_shift);
  ...
  // Save tos_state before daddu overwrites index (T0 aliases tos_state).
  // After the volatile check above, flags (T1) is no longer needed — reuse it.
  __ move(flags, tos_state);     // T1 = saved type selector
  const Register type = flags;   // T1 holds the field type for dispatch below

  if (!is_static) pop_and_check_object(obj);
  __ daddu(index, obj, off);     // T0 now = field address (tos_state/index overwritten)
  ...
  // type (T1) has the field type from ResolvedFieldEntry::type_offset()
  __ bne(type, R0, notByte);
  __ delayed()->nop();
...
// The registers cache and index expected to be set before call.
// cache must point to a ResolvedFieldEntry.
void TemplateTable::jvmti_post_field_mod(Register cache, Register index, bool is_static) {
  ...
  if (JvmtiExport::can_post_field_modification()) {
    ...
    if (is_static) {
      ...
      __ lbu(tmp3, Address(tmp2, in_bytes(ResolvedFieldEntry::type_offset())));
      __ move(tmp1, SP);
      __ move(AT, ltos);
...
void TemplateTable::putfield_or_static(int byte_no, bool is_static, RewriteControl rc) {
  ...
  // btos
  // tos_state has the type directly from ResolvedFieldEntry::type_offset()
  __ bne(tos_state, R0, notByte);
  __ delayed()->nop();
...
void TemplateTable::jvmti_post_fast_field_mod() {
  if (JvmtiExport::can_post_field_modification()) {
    ...
    __ move(tmp3, SP);
    // access constant pool cache entry (ResolvedFieldEntry)
...
void TemplateTable::fast_storefield(TosState state) {
  ...
  // access constant pool cache (ResolvedFieldEntry)
  __ load_field_entry(T3, T2);

  // Get flags and field offset from ResolvedFieldEntry
  __ lbu(T1, Address(T3, in_bytes(ResolvedFieldEntry::flags_offset())));
  __ lw(T2, Address(T3, in_bytes(ResolvedFieldEntry::field_offset_offset())));
...
void TemplateTable::fast_accessfield(TosState state) {
  ...
  // Get field offset from ResolvedFieldEntry
  __ lw(T2, Address(T3, in_bytes(ResolvedFieldEntry::field_offset_offset())));

  {
    __ lbu(AT, Address(T3, in_bytes(ResolvedFieldEntry::flags_offset())));
    __ move(scratch, 1 << ResolvedFieldEntry::is_volatile_shift);
...
void TemplateTable::fast_xaccess(TosState state) {
  ...
  // get receiver
  __ ld(T1, aaddress(0));
  // access constant pool cache (ResolvedFieldEntry, at BCP offset 2)
...
void TemplateTable::prepare_invoke(int byte_no,
  ...
  if (is_invokedynamic || is_invokehandle) {
   Label L_no_push;
     // ResolvedIndyEntry::has_appendix_shift=1 vs ResolvedMethodEntry::has_appendix_shift=3
     const int appendix_bit = is_invokedynamic ? (1 << ResolvedIndyEntry::has_appendix_shift)
                                               : (1 << ResolvedMethodEntry::has_appendix_shift);
     __ move(AT, appendix_bit);
     __ andr(AT, AT, flags);
  ...
  if (load_receiver) {
    // Load number of parameters from ResolvedMethodEntry/ResolvedIndyEntry (T3)
    if (is_invokedynamic) {
      __ lhu(recv, Address(T3, in_bytes(ResolvedIndyEntry::num_parameters_offset())));
    } else {
      __ lhu(recv, Address(T3, in_bytes(ResolvedMethodEntry::num_parameters_offset())));
    }
...
void TemplateTable::invokevirtual_helper(Register index,
  ...
  Label notFinal;
  __ move(AT, (1 << ResolvedMethodEntry::is_vfinal_shift));
  __ andr(AT, flags, AT);
  ...
  __ ld(method, AT, base + in_bytes(vtableEntry::method_offset()));
  __ profile_arguments_type(T2, method, T9, true);
  __ jump_from_interpreted(method);
...
void TemplateTable::invokeinterface(int byte_no) {
  ...
  __ move(AT, (1 << ResolvedMethodEntry::is_forced_virtual_shift));
  __ andr(AT, T1, AT);
...
void TemplateTable::_new() {
  ...
  // supports_inline_contig_alloc removed in jdk25u; Eden inline alloc disabled
  ...
  if(allow_shared_alloc){
    ...
    Label done, retry;
    Address heap_top(T1);
    __ set64(T1, 0 /* top_addr removed in jdk25u */);
    __ ld(FSR, heap_top);

    __ bind(retry);
    __ set64(AT, 0 /* end_addr removed in jdk25u */);
    __ ld(AT, AT, 0);
    __ daddu(T2, FSR, T0);
    __ slt(AT, AT, T2);
    __ bne(AT, R0, slow_case);
  ...
  if (UseTLAB || false /* supports_inline_contig_alloc removed */) {
...

    {
      ...
      __ push(atos);
      __ call_VM_leaf(
           CAST_FROM_FN_PTR(address, ((int(*)(oopDesc*))SharedRuntime::dtrace_object_alloc)), FSR);
      __ pop(atos);
...
void TemplateTable::monitorenter() {
  ...
  // find a free slot in the monitor block (result in c_rarg0)
  {
    ...
    // free slot?
    __ bind(loop);
    __ ld(AT, T2, in_bytes(BasicObjectLock::obj_offset()));
  ...
  __ daddiu(BCP, BCP, 1);
  __ sd(FSR, c_rarg0, in_bytes(BasicObjectLock::obj_offset()));
...
void TemplateTable::monitorexit() {
  ...
  // find matching slot
  {
    ...
    __ bind(loop);
    __ ld(AT, c_rarg0, in_bytes(BasicObjectLock::obj_offset()));
    __ beq(FSR, AT, found);
    __ delayed()->nop();
    __ daddiu(c_rarg0, c_rarg0, entry_size);
    __ bind(entry);
