...
class InterpreterMacroAssembler: public MacroAssembler {
 ...
 public:
  ...
  void restore_locals() {
    // LVP is stored relativized (word offset from FP); decode: LVP = FP + n * wordSize
    ld(LVP, FP, frame::interpreter_frame_locals_offset * wordSize);
    dsll(LVP, LVP, LogBytesPerWord);  // n * wordSize
    daddu(LVP, LVP, FP);
  }
  ...
  void get_constant_pool_cache(Register reg) {
    ...
    ld(reg, reg, in_bytes(ConstantPool::cache_offset()));
  }

  void get_cpool_and_tags(Register cpool, Register tags) {
    get_constant_pool(cpool);
    ld(tags, cpool, in_bytes(ConstantPool::tags_offset()));
  }
