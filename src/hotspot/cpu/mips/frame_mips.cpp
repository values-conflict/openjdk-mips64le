...
bool frame::safe_for_sender(JavaThread *thread) {
  ...
  if (_cb != NULL ) {
    ...
    // is_zombie/is_unloaded removed in JDK25 -- skip zombie check
...
// frame_size() is now inline in frame.hpp for jdk25u
...
// optimized_entry_frame_is_first removed in jdk25u
...
// OptimizedEntryBlob methods not needed on MIPS (Panama FFI Phase 3)
...
// In release builds this is inlined as empty via NOT_DEBUG_RETURN in frame_mips.hpp.
// Only define the debug body here.
#ifdef ASSERT
...
// update_map_with_saved_link is now a template in frame_mips.inline.hpp (jdk25u)

// sender_for_compiled_frame is now inline in frame_mips.inline.hpp

// frame::sender is now in frame_mips.inline.hpp (jdk25u structure)
...
JavaThread** frame::saved_thread_address(const frame& f) {
  // Not used without C2 or C1 on MIPS
  ShouldNotCallThis();
  return nullptr;
}
