  ...
  public:
    MontgomeryMultiplyGenerator (Assembler *as, bool squaring)
      ...
      if (squaring)
        ...
        Pb_base = reg->successor();
      Pn_base = reg->successor();
      Rlen = reg->successor();
      inv = reg->successor();
      Rlen2 = inv;        // Reuse inv
      Pm_base = reg->successor();

                          // Working registers:
      Ra = reg->successor();         // The current digit of a, b, n, and m.
      Rb = reg->successor();
      Rm = reg->successor();
      Rn = reg->successor();

      Iam = reg->successor();        // Index to the current/next digit of a, b, n, and m.
      Ibn = reg->successor();

      if (squaring) {
        t0 = reg->successor();       // Three registers which form a
...
void StubGenerator_generate(CodeBuffer* code, StubGenBlobId blob_id) {
  if (UnsafeMemoryAccess::_table == nullptr) {
    UnsafeMemoryAccess::create_table(4); // 2 for copyMemory; room for future additions
  }
