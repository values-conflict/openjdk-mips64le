...
static void preserve_callee_saved_registers(MacroAssembler* _masm, const ABIDescriptor& abi, int reg_save_area_offset) {
  ...
  for (int i = 0; i < Register::number_of_registers; i++) {
    Register reg = as_Register(i);
    if (reg == FP || reg == SP || reg == RA) continue;
    if (!abi.is_volatile_reg(reg)) {
...
address UpcallLinker::make_upcall_stub(jobject receiver, Symbol* signature,
  ...
  // allocate frame (frame_size is aligned, so stack stays aligned)
  __ li(T8, frame_size);
  __ dsubu(SP, SP, T8);

  // always spill args since we need to call on_entry to get the thread
  arg_spiller.generate_spill(_masm, arg_save_area_offset);
  preserve_callee_saved_registers(_masm, abi, reg_save_area_offset);

  __ block_comment("{ on_entry");
  // on_entry(UpcallStub::FrameData*) -> JavaThread*
  __ li(AT, frame_data_offset);
  __ daddu(RA0, SP, AT);
  ...
  if (needs_return_buffer) {
    ...
    __ li(AT, ret_buf_offset);
    __ daddu(as_Register(locs.get(StubLocations::RETURN_BUFFER)), SP, AT);
  }
  arg_shuffle.generate(_masm, as_VMStorage(shuffle_reg), abi._shadow_space_bytes, 0);
  __ block_comment("} argument shuffle");

  __ block_comment("{ load target ");
  // RT0 = T0 = r12 = j_rarg0: load receiver jobject, then resolve to Method*
  __ li(RT0, (intptr_t)receiver);
  __ call(StubRoutines::upcall_stub_load_target(), relocInfo::runtime_call_type);
  __ delayed()->nop(); // puts target Method* in Rmethod
  __ block_comment("} load target ");

  __ push_cont_fastpath(TREG);

  // call compiled method: load entry from Method* and jump
  __ ld(T8, Rmethod, in_bytes(Method::from_compiled_offset()));
  __ jalr(T8);
  __ delayed()->nop();
  ...
  if (!needs_return_buffer) {
    ...
    if (call_regs._ret_regs.length() == 1) { // 0 or 1
      ...
      switch (ret_type) {
        ...
        case T_FLOAT:
        case T_DOUBLE:
          j_expected_result_reg = as_VMStorage(F0);
          break;
        default:
          fatal("unexpected return type: %s", type2name(ret_type));
      }
      // No need to move for now; at least assert they are the same
    ...
    for (int i = 0; i < call_regs._ret_regs.length(); i++) {
      ...
      if (reg.type() == StorageType::INTEGER) {
        __ ld(as_Register(reg), T8, offset);
        offset += 8;
      } else if (reg.type() == StorageType::FLOAT) {
        __ ldc1(as_FloatRegister(reg), T8, offset);
        offset += 8;
      } else {
        ShouldNotReachHere();
      }
